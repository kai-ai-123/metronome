...
- 拍子セレクト: 8種類（2/4, 3/4, 4/4, 5/4, 3/8, 6/8, 9/8, 12/8） → `components/TimeSignatureSelect.tsx`
...
- 音色選択: 4種類（クリック/ウッド/ビープ/ハイハット）のボタン切り替え → `components/SoundSelect.tsx`
- プリセット: 保存ボタン + 呼び出しボタン
...
| 音色選択 | 4種類から選択（クリック/ウッド/ビープ/ハイハット） |
